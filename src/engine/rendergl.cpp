...
void findorientation()
{
    ...
    vecfromyawpitch(yaw, pitch, 1, 0, camdir);
    ...
    vec dir1 = invmvpmatrix.perspectivetransform(vec(x*2-1, 1-2*y, 2-1)),
    dir2 = invmvpmatrix.perspectivetransform(vec(x*2-1, 1-2*y, -1));

    lookdir = dir1.sub(dir2).normalize();

    if(raycubepos(camera1->o, lookdir, worldpos, 0, RAY_CLIPMAT|RAY_SKIPFIRST) == -1)
...
FVARP(thirdpersondistance, 0, 20, 1000);
...
void loadcrosshair(const char *name, int i)
{
    ...
    if(crosshairs[i] == notexture)
        crosshairs[i] = textureload(game::defaultcrosshair(i) ? game::defaultcrosshair(i) : "data/crosshair", 3, true);
}
...
SVARP(guicursortex, "newui/cursor");
...
void renderbackground(const char *caption, Texture *mapshot, const char *mapname, const char *mapinfo, bool restore, bool force)
{
    ...
    loopi(restore ? 1 : 3)
    {
        ...
        Texture *background = textureload("data/loadingscreen/load_back.png", 3, false, false);
        glBindTexture(GL_TEXTURE_2D, background->id);

        ratio = max((float)h/background->ys, (float)w/background->xs);

        int wborder = (w - background->xs*ratio) / 2;
        int hborder = (h - background->ys*ratio) / 2;

        glBegin(GL_TRIANGLE_STRIP);
            glTexCoord2f(0,0); glVertex2f(wborder,   hborder);
            glTexCoord2f(1,0); glVertex2f(w-wborder, hborder);
            glTexCoord2f(0,1); glVertex2f(wborder,   h-hborder);
            glTexCoord2f(1,1); glVertex2f(w-wborder, h-hborder);
        glEnd();

        /*module logo*/
        float logoh = 0.30f*min(w, h), logow = logoh*0.90;
        float logox = 0.18f*(w - logow), logoy = 1.35*(h*0.5f - logoh);

        settexture(imagelogo, 3);

        glBegin(GL_TRIANGLE_STRIP);
            glTexCoord2f(0,0); glVertex2f(logox,       logoy);
            glTexCoord2f(1,0); glVertex2f(logox+logow, logoy);
            glTexCoord2f(0,1); glVertex2f(logox,       logoy+logoh);
            glTexCoord2f(1,1); glVertex2f(logox+logow, logoy+logoh);
        glEnd();

        /*loading screen title*/
        float lh = 0.23f*min(w,h), lw = lh*5.67;
        float lx = 0.03f*(w - lw), ly = 0.03f*(h*0.5f - lh);

        settexture("data/loadingscreen/title.png", 3);
        ...
        float badgex = 0.01f*(w - badgew), badgey = 2.3f*(h*0.5f - badgeh);

        settexture("data/loadingscreen/engine_badge.png", 3);

        glBegin(GL_TRIANGLE_STRIP);
            glTexCoord2f(0, 0); glVertex2f(badgex,          badgey);
            glTexCoord2f(1, 0); glVertex2f(badgex+badgew,   badgey);
            glTexCoord2f(0, 1); glVertex2f(badgex,          badgey+badgeh);
            glTexCoord2f(1, 1); glVertex2f(badgex+badgew,   badgey+badgeh);
        glEnd();
        ...
        if(caption)
        {
            ...
            float ty = h - 0.075f*1.5f*min(w, h) - 1.25f*FONTH*tsz;
        ...
        if(mapshot || mapname)
        {
            ...
            if(mapinfo)
            {
                ...
                text_bounds(mapinfo, miw, mih, infowidth);
                x-= 0.5*(miw*msz + FONTH*msz);
            }
            ...
            if(mapname)
            {
                ...
                float tx = 0.9f*sz - tw*tsz;
                float ty = 0.9f*sz - FONTH*tsz;
        ...
        if(!mapshot && !mapname && !caption && !mapinfo)
        {
            float scale = (min(w, h) * 0.100f) / max(1, text_width(version)),
                ty = w - (min(w,h) * 0.15f),
                tx = h - (1.5f * FONTH * scale);
