...
#endif
{
    ...
    void preload()
    {
        static const char *basemodels[3] = { "banana", "banana/red", "banana/blue" };
    ...
    void rendergame()
    {
        ...
        {
            baseinfo &b = bases[i];
            const char *flagname = b.owner[0] ? (strcmp(b.owner, player1->team) ? "banana/red" : "banana/blue") : "banana";
            rendermodel(&b.light, flagname, ANIM_MAPMODEL|ANIM_LOOP, b.o, 0, 0, 0, MDL_SHADOW | MDL_CULL_VFC | MDL_CULL_OCCLUDED);
...
//            particle_fireball(b.ammopos, 4.8f, PART_EXPLOSION, 0, b.owner[0] ? (strcmp(b.owner, player1->team) ? 0x802020 : 0x2020FF) : 0x208020, 4.8f);
            ...
            if(b.ammotype>0 && b.ammotype<=I_CARTRIDGES-I_SHELLS+1)
            {
                ...
                if(m_regencapture)
                {
                    ...
                    rendermodel(&b.light, ammoname, ANIM_MAPMODEL|ANIM_LOOP, ammopos, lastmillis/10.0f, 0, 0, MDL_SHADOW | MDL_CULL_VFC | MDL_CULL_OCCLUDED);
                }
                else loopj(b.ammo)
                {
                    ...
                    ammopos.z += 4;
                    rendermodel(&b.light, entities::entmdlname(I_SHELLS+b.ammotype-1), ANIM_MAPMODEL|ANIM_LOOP, ammopos, 0, 0, 0, MDL_SHADOW | MDL_CULL_VFC | MDL_CULL_OCCLUDED);
                }
            }
    ...
    void drawhud(fpsent *d, int w, int h)
    {
        ...
        settexture("data/fps/hud/radar.png", 3);
        drawradar(x - roffset, y - roffset, rsize);
        #if 0
        settexture("data/fps/hud/compass.png", 3);
        glPushMatrix();
        ...
        else settexture("data/fps/hud/blip_blue.png", 3);
        glPushMatrix();
        ...
        else settexture("data/fps/hud/blip_grey.png", 3);
        drawblips(d, blipsize, fw, fh, 0, showenemies);
        if(basenumbers) setfont("digit_red");
        else settexture("data/fps/hud/blip_red.png", 3);
        drawblips(d, blipsize, fw, fh, -1, showenemies);
    ...
    void setup()
    {
        ...
        loopv(entities::ents)
        {
            ...
            b.o = e->o;
            b.ammopos = b.o;
            abovemodel(b.ammopos, "banana");
            b.ammopos.z += AMMOHEIGHT-2;
            b.ammotype = e->attr[0];
            defformatstring(alias)("base_%d", e->attr[1]);
    ...
    int pickteamspawn(const char *team)
    {
        ...
        {
            extentity *e = entities::ents[i];
            if(e->type!=PLAYERSTART || e->attr[1]) continue;
