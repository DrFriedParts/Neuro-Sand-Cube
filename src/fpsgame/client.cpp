...
namespace game
{
    ...
    int getclientaccuracy(int cn)
    {
        fpsent *d = getclient(cn);
        return d ? d->totaldamage * 100 / max(d->totalshots, 1) : 0;
    }
    ICOMMAND(getclientaccuracy, "i", (int *cn), intret(getclientaccuracy(*cn)));
    ...
    {
        int score = 0;
        if(cmode) score = cmode->getteamscore(team);
        else if(m_teammode) loopv(players)
        {
            if(!strcmp(team, players[i]->team))
                score += players[i]->frags;
        }
        return score;
    }
    ...
    string clientmap = "";
    ICOMMAND(getcurmap, "", () , result(clientmap));
    ...
    void sayteam(char *text) { conoutf(CON_TEAMCHAT, "<%s>\f1 %s", colorname(player1), text); addmsg(N_TEXT, "rcis", player1, CHAT_TEAM, text); }
    COMMAND(sayteam, "C");

    void emote(char *text) { conoutf(CON_CHAT, "\f6* %s %s", colorname(player1), text); addmsg(N_TEXT, "rcis", player1, CHAT_TALK|CHAT_EMOTE, text); }
    COMMANDN(me, emote, "C");

    void teamemote(char *text) { conoutf(CON_TEAMCHAT, "\f5* %s %s", colorname(player1), text); addmsg(N_TEXT, "rcis", player1, CHAT_TEAM|CHAT_EMOTE, text); }
    ...
    void parsemessages(int cn, fpsent *d, ucharbuf &p)
    {
        ...
        while(p.remaining()) switch(type = getint(p))
        {
            ...
            case N_TEXT:
            {
                ...
                if(isignored(t->clientnum)) break;
            ...
            case N_ATTACH:
            {
                ...
                if(d)
                {
                    d->attached.add(part,text,animrule);
                }
                break;
            }
    ...
    {
        string fname = "packages/base/fps/getmap.ogz";
        string cname = "packages/base/fps/getmap-art.cfg";
        ...
        switch(type)
        {
            ...
            {
                if(!m_edit) return;
                stream *cfg = openrawfile(path(cname), "wb");
                if(!cfg) return;
                cfg->write(data, len);
                delete cfg;
                break;
            ...
            case N_SENDMAP:
            {
                ...
                map->write(data, len);
                delete map;
                if(load_world("getmap", oldname[0] ? oldname : NULL))
                    entities::spawnitems(true);
...
/* THIS IS ONLY A PORTION OF IT
    ...
    */
    ...
    void sendmap()
    {
        ...
        defformatstring(fname)("packages/base/fps/%s.ogz", mname);
        defformatstring(cname)("packages/base/fps/%s-art.cfg", mname);
        ...
        if(map && cfg)
        {
            ...
            else
            {
                ...
                sendfile(-1, 2, map, "ri", N_UPLOADMAP);
                if(needclipboard >= 0) needclipboard++;
            }
            delete cfg;
            delete map;
    ...
    {
        player1->attached.add(p,m,r);
        addmsg(N_ATTACH, "risi", p, m, r);
    }

    void playerdetach(int p)
    {
        player1->attached.remove(p);
        addmsg(N_DETACH, "ri", p);
    }
