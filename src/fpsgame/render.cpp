...
namespace game
{
    ...
    VARFP(playermodel, 0, 0, 7, { if(player1->clientnum < 0) player1->playermodel = playermodel; });
    ...
    {
        { "captaincannon", "captaincannon/blue", "captaincannon/red", "captaincannon/hudguns", NULL, NULL, { NULL, NULL, NULL }, "captaincannon", "captaincannon_blue", "captaincannon_red", true },
        { "captaincannon/blue", "captaincannon/blue", "captaincannon/red", NULL, "captaincannon/hudguns", NULL, { NULL, NULL, NULL }, "captaincannon_blue", "captaincannon_blue", "captaincannon_red", true },
        { "captaincannon/red", "captaincannon/blue", "captaincannon/red", NULL, "captaincannon/hudguns", NULL, { NULL, NULL, NULL }, "captaincannon_red", "captaincannon_blue", "captaincannon_red", true },
    ...
    void renderplayer(fpsent *d, const playermodelinfo &mdl, int team, float fade, bool mainpass)
    {
        ...
        int ai = 0;
        //modelattach a[d->attached.numattachments() + 5];
        int num = d->attached.numattachments() + 5;
        modelattach* a = new modelattach[num];

        attachlist &attachments = d->attached;
        loopv(attachments.list)
        {
            int animrule;
            int animtime;
            const char* tag = attachtag(attachments.list[i]->part);
            switch (attachments.list[i]->animrule)
            {
            ...
            a[ai++] = modelattach(tag, attachments.list[i]->model, animrule, animtime);
        }
    ...
    void drawhudmodel(fpsent *d, int anim, float speed = 0, int base = 0)
    {
        ...
        rendermodel(NULL, gunname, anim, sway, testhudgun ? 0 : d->yaw+90, testhudgun ? 0 : d->pitch, d->roll, MDL_LIGHT|MDL_HUD, interp, a, base, (int)ceil(speed));
    }
